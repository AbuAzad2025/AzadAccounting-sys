...
from models import (
    Account, AuditLog, Branch, Site, Check, CheckStatus, Currency, Customer, Employee, ExchangeRate, ExchangeTransaction, Expense, ExpenseType, GLBatch,
    ...
    PreOrder, Product, ProductCategory, ProductPartner, Role, Sale, SaleLine, ServicePart, ServiceRequest, ServiceStatus, ServiceTask,
    Shipment, ShipmentItem, StockAdjustment, StockAdjustmentItem, StockLevel, Supplier,
    SupplierSettlement, Transfer, TransferDirection, Warehouse, WarehousePartnerShare, WarehouseType, _ensure_customer_for_counterparty, _gl_upsert_batch_and_entries,
...
@click.command("seed-deep-demo")
@click.option("--customers", type=int, default=8)
@click.option("--suppliers", type=int, default=4)
@click.option("--partners", type=int, default=4)
@click.option("--products", type=int, default=12)
@click.option("--sales", type=int, default=8)
@click.option("--services", type=int, default=6)
@click.option("--shipments", type=int, default=4)
@click.option("--expenses", type=int, default=8)
@click.option("--payments", type=int, default=16)
@click.option("--checks", type=int, default=8)
@click.option("--preorders", type=int, default=6)
@click.option("--invoices", type=int, default=6)
@click.option("--days-ago", type=int, default=0)
@click.option("--tag", type=str, default="")
@with_appcontext
def seed_deep_demo(customers: int, suppliers: int, partners: int, products: int, sales: int, services: int, shipments: int, expenses: int, payments: int, checks: int, preorders: int, invoices: int, days_ago: int, tag: str) -> None:
    now = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(days=int(days_ago or 0))
    seed_tag = (tag or "").strip() or uuid.uuid4().hex[:8].upper()
    seed_ref_tag = f"{seed_tag}-{uuid.uuid4().hex[:4].upper()}"
    cust_n = max(int(customers or 0), 0)
    supp_n = max(int(suppliers or 0), 0)
    part_n = max(int(partners or 0), 0)
    prod_n = max(int(products or 0), 0)
    sale_n = max(int(sales or 0), 0)
    srv_n = max(int(services or 0), 0)
    shp_n = max(int(shipments or 0), 0)
    exp_n = max(int(expenses or 0), 0)
    pay_n = max(int(payments or 0), 0)
    chk_n = max(int(checks or 0), 0)
    pre_n = max(int(preorders or 0), 0)
    inv_n = max(int(invoices or 0), 0)

    def _phone(seed_base: int, idx: int) -> str:
        num = (seed_base + idx) % 10000000
        return f"059{num:07d}"

    def _email(prefix: str, idx: int) -> str:
        return f"{prefix}{seed_tag.lower()}{idx}@seed.local"

    def _ref(prefix: str, idx: int) -> str:
        return f"{prefix}-{seed_ref_tag}-{idx:03d}"

    def _unique_phone(model, raw: str) -> str:
        phone = raw
        for _ in range(20):
            if not model.query.filter(model.phone == phone).first():
                return phone
            try:
                phone = phone[:3] + f"{(int(phone[3:]) + 1) % 10000000:07d}"
            except Exception:
                phone = f"059{uuid.uuid4().int % 10000000:07d}"
        return f"059{uuid.uuid4().int % 10000000:07d}"

    def _unique_partner_phone(raw: str) -> str:
        phone = raw
        for _ in range(20):
            if not Partner.query.filter(Partner.phone_number == phone).first():
                return phone
            try:
                phone = phone[:3] + f"{(int(phone[3:]) + 1) % 10000000:07d}"
            except Exception:
    ...
    seed_base = int(uuid.uuid4().int % 10000000)
    created_customers: list[Customer] = []
    created_suppliers: list[Supplier] = []
    created_partners: list[Partner] = []
    created_products: list[Product] = []
    created_sales: list[Sale] = []
    created_services: list[ServiceRequest] = []
    created_shipments: list[Shipment] = []
    created_expenses: list[Expense] = []
    created_invoices: list[Invoice] = []
    created_preorders: list[PreOrder] = []

    try:
        with _begin():
            from models import CURRENCY_CHOICES
            currency_map = {code: name for code, name in CURRENCY_CHOICES}
            for code in {"ILS", "USD", "EUR", "JOD"}:
                if not Currency.query.filter(Currency.code == code).first():
                    db.session.add(Currency(code=code, name=currency_map.get(code, code), symbol=code, decimals=2, is_active=True))

            for base, rate in [("USD", "3.65"), ("EUR", "3.95"), ("JOD", "5.15")]:
                exists = ExchangeRate.query.filter(
                    ExchangeRate.base_code == base,
                    ExchangeRate.quote_code == "ILS",
                    ExchangeRate.is_active.is_(True),
                ).first()
                if not exists:
                    db.session.add(ExchangeRate(base_code=base, quote_code="ILS", rate=Decimal(str(rate)), valid_from=now - timedelta(days=30), source="seed", is_active=True))

            branch_code = f"BR{seed_tag}".strip()
            branch = Branch.query.filter(func.lower(Branch.code) == branch_code.lower()).order_by(Branch.id.asc()).first()
            if not branch:
                branch = Branch.query.filter(Branch.is_active.is_(True)).order_by(Branch.id.asc()).first() or Branch.query.order_by(Branch.id.asc()).first()
            if not branch:
                branch = Branch(name=f"فرع {seed_tag}", code=f"BR{seed_tag}", is_active=True)
                db.session.add(branch)
                db.session.flush()

            site_code = f"S{seed_tag}".strip()
            site = Site.query.filter(func.lower(Site.code) == site_code.lower(), Site.branch_id == branch.id).order_by(Site.id.asc()).first()
            if not site:
                site = Site.query.filter(Site.branch_id == branch.id).order_by(Site.id.asc()).first()
            if not site:
                site = Site(code=f"S{seed_tag}", name=f"موقع {seed_tag}", branch_id=branch.id, is_active=True)
                db.session.add(site)
                db.session.flush()

            warehouse_main = Warehouse.query.filter(Warehouse.is_active.is_(True), Warehouse.warehouse_type == WarehouseType.MAIN.value).order_by(Warehouse.id.asc()).first()
            if not warehouse_main:
                warehouse_main = Warehouse(name=f"مستودع رئيسي {seed_tag}", warehouse_type=WarehouseType.MAIN.value, branch_id=branch.id, is_active=True)
                db.session.add(warehouse_main)
                db.session.flush()

            user = User.query.filter(User.is_active.is_(True)).order_by(User.id.asc()).first() or User.query.order_by(User.id.asc()).first()
            if not user:
                admin_role = _get_or_create_role("admin")
                user = _get_or_create_user(ADMIN_USERNAME, ADMIN_EMAIL, ADMIN_PASSWORD, admin_role)
                db.session.flush()

            for i in range(cust_n):
                cust_email = _email("cust", i)
                cust = Customer.query.filter(Customer.email == cust_email).first()
                if not cust:
                    cust_phone = _unique_phone(Customer, _phone(seed_base, i))
                    cust = Customer(
                        name=f"عميل بذور {seed_tag} {i + 1}",
                        phone=cust_phone,
                        email=cust_email,
                        address="رام الله",
                        category="عادي",
            ...
            for i in range(supp_n):
                sup_email = _email("supp", i)
                sup = Supplier.query.filter(Supplier.email == sup_email).first()
                if not sup:
                    sup_phone = _unique_phone(Supplier, _phone(seed_base + 2000, i))
                    sup = Supplier(
                        name=f"مورد بذور {seed_tag} {i + 1}",
                        phone=sup_phone,
                        email=sup_email,
                        address="غزة",
                        is_local=bool(i % 2 == 0),
                        currency="USD" if i % 2 else "ILS",
                        identity_number=_ref("SUP", i),
                    )
                    db.session.add(sup)
            ...
            for i in range(part_n):
                par_email = _email("partner", i)
                par = Partner.query.filter(Partner.email == par_email).first()
                if not par:
                    par_phone = _unique_partner_phone(_phone(seed_base + 5000, i))
                    par = Partner(
                        name=f"شريك بذور {seed_tag} {i + 1}",
                        phone_number=par_phone,
                        email=par_email,
                        address="الخليل",
                        share_percentage=_Q2(10 + (i * 5)),
                        currency="ILS",
                        identity_number=_ref("PAR", i),
                    )
                    db.session.add(par)
                created_partners.append(par)

            _flush("partners")

            partner_wh = Warehouse.query.filter(Warehouse.is_active.is_(True), Warehouse.warehouse_type == WarehouseType.PARTNER.value).order_by(Warehouse.id.asc()).first()
            if not partner_wh and created_partners:
                partner_wh = Warehouse(
                    name=f"مستودع شريك {seed_tag}",
                    warehouse_type=WarehouseType.PARTNER.value,
                    branch_id=branch.id,
                    partner_id=created_partners[0].id,
                    share_percent=_Q2(30),
                )
                db.session.add(partner_wh)
                db.session.flush()
            ...
            for i in range(prod_n):
                ...
                price = _Q2(80 + (i * 7))
                prod_name = f"منتج بذور {seed_tag} {i + 1}"
                prod = Product.query.filter(func.lower(Product.name) == prod_name.lower(), Product.warehouse_id == warehouse_main.id).first()
                if not prod:
                    prod = Product(
                        name=prod_name,
                        price=price,
                        selling_price=price,
                        purchase_price=_Q2(50 + i * 3),
                        unit_price_before_tax=price,
                        tax_rate=_Q2(0),
                        currency=cur,
                        warehouse_id=warehouse_main.id,
                        supplier_id=created_suppliers[i % len(created_suppliers)].id if created_suppliers else None,
            ...
            if created_partners and created_products:
                for i, par in enumerate(created_partners):
                    prod = created_products[i % len(created_products)]
                    pct = _Q2(15 + (i * 5))
                    db.session.add(ProductPartner(product_id=prod.id, partner_id=par.id, share_percent=float(pct)))
                    exists_share = WarehousePartnerShare.query.filter(
                        WarehousePartnerShare.partner_id == par.id,
                        WarehousePartnerShare.warehouse_id == warehouse_main.id,
                        WarehousePartnerShare.product_id == prod.id,
                    ).first()
                    if not exists_share:
                        db.session.add(WarehousePartnerShare(partner_id=par.id, warehouse_id=warehouse_main.id, product_id=prod.id, share_percentage=float(pct)))
            _flush("partner_shares")

            for i in range(pre_n):
                prod = created_products[i % len(created_products)] if created_products else None
                if not prod or not created_customers:
                    break
                preorder = PreOrder(
                    product_id=prod.id,
                    warehouse_id=warehouse_main.id,
                    ...
                    supplier_id=created_suppliers[i % len(created_suppliers)].id if created_suppliers else None,
                    partner_id=created_partners[i % len(created_partners)].id if created_partners else None,
                    prepaid_amount=_Q2(20 + (i * 5)),
                    tax_rate=_Q2(0),
                    payment_method=PaymentMethod.CASH.value,
                    notes=f"Seed preorder {seed_tag}",
                    preorder_date=now - timedelta(days=i),
            ...
            for i in range(sale_n):
                ...
                sale = Sale(
                    ...
                    tax_rate=_Q2(0),
                    discount_total=_Q2(0),
                    shipping_cost=_Q2(10 + i),
                    notes=f"Seed sale {seed_tag}",
                )
                db.session.add(sale)
                db.session.flush()
                for j in range(1 + (i % 2)):
                    prod = created_products[(i + j) % len(created_products)]
                    db.session.add(SaleLine(
                        sale_id=sale.id,
                        product_id=prod.id,
                        warehouse_id=warehouse_main.id,
                        quantity=1 + (j % 3),
                        unit_price=_Q2(100 + (i * 4) + (j * 2)),
                        discount_rate=_Q2(0),
                        tax_rate=_Q2(0),
                        line_receiver=f"مستلم {seed_tag}",
                    ))
            ...
            for i in range(srv_n):
                if not created_customers or not created_products:
                    break
                srv = ServiceRequest(
                    service_number=_ref("SRV", i),
                    customer_id=created_customers[i % len(created_customers)].id,
                    tax_rate=_Q2(0),
                    discount_total=_Q2(0),
                    currency=currencies[i % len(currencies)],
                    status=ServiceStatus.COMPLETED.value,
                    notes=f"Seed service {seed_tag}",
                    description=f"Seed service {seed_tag}",
                ...
                db.session.add(ServicePart(
                    ...
                    tax_rate=_Q2(0),
                    partner_id=created_partners[i % len(created_partners)].id if created_partners else None,
                    share_percentage=_Q2(10) if created_partners else _Q2(0),
                ))
                db.session.add(ServiceTask(
                    service_id=srv.id,
                    description=f"عمل صيانة {seed_tag}",
                    quantity=1,
                    unit_price=_Q2(40 + i),
                    discount=_Q2(0),
                    tax_rate=_Q2(0),
                    partner_id=created_partners[i % len(created_partners)].id if created_partners else None,
                    share_percentage=_Q2(5) if created_partners else _Q2(0),
                ))
            ...
            for i in range(inv_n):
                if not created_customers:
                    break
                inv = Invoice(
                    invoice_number=_ref("INV", i),
                    invoice_date=now - timedelta(days=i),
                    customer_id=created_customers[i % len(created_customers)].id,
                    currency=currencies[i % len(currencies)],
                    notes=f"Seed invoice {seed_tag}",
                    source="MANUAL",
                )
                db.session.add(inv)
                db.session.flush()
                for j in range(1 + (i % 2)):
                    db.session.add(InvoiceLine(
                        invoice_id=inv.id,
                        description=f"بند فاتورة {seed_tag}",
                        quantity=1 + (j % 2),
                        unit_price=_Q2(120 + (i * 5)),
                        tax_rate=_Q2(0),
                        discount=_Q2(0),
                        product_id=created_products[(i + j) % len(created_products)].id if created_products else None,
                    ))
            ...
            for i in range(shp_n):
                if not created_products:
                    break
                shp = Shipment(
                    shipment_date=now - timedelta(days=i * 2),
                    origin="تركيا",
                    destination="رام الله",
                    destination_id=warehouse_main.id,
                    status="ARRIVED",
                    currency="USD",
                    shipping_cost=_Q2(200 + i * 10),
                    customs=_Q2(50 + i * 5),
                    vat=_Q2(20 + i * 2),
                    insurance=_Q2(10 + i),
                    notes=f"Seed shipment {seed_tag}",
                )
                db.session.add(shp)
                db.session.flush()
                for j in range(1 + (i % 2)):
                    ...
                    db.session.add(ShipmentItem(
                        ...
                        warehouse_id=warehouse_main.id,
                        quantity=2 + (j % 3),
                        unit_cost=_Q2(60 + (i * 4) + (j * 2)),
                        declared_value=_Q2(65 + (i * 4) + (j * 2)),
                    ))
                if created_partners:
                    par = created_partners[i % len(created_partners)]
                    db.session.add(ShipmentPartner(
                        shipment_id=shp.id,
                        partner_id=par.id,
                        unit_price_before_tax=_Q2(500 + i * 50),
                        share_percentage=_Q2(20),
                        notes=f"Seed shipment partner {seed_tag}",
                    ))
                created_shipments.append(shp)

            _flush("shipments")

            expense_type_id = _get_expense_type_id("متفرقات")
            for i in range(exp_n):
                exp = Expense(
                    ...
                    branch_id=branch.id,
                    site_id=site.id,
                    customer_id=created_customers[i % len(created_customers)].id if created_customers else None,
                    supplier_id=created_suppliers[i % len(created_suppliers)].id if created_suppliers else None,
                    partner_id=created_partners[i % len(created_partners)].id if created_partners else None,
                    shipment_id=created_shipments[i % len(created_shipments)].id if created_shipments else None,
                    payee_type="OTHER",
                    payee_name=f"جهة {seed_tag}",
                    payment_method="cash",
                    description=f"Seed expense {seed_tag}",
                    notes=f"Seed expense {seed_tag}",
                )
                db.session.add(exp)
            ...
            methods = [PaymentMethod.CASH.value, PaymentMethod.CARD.value, PaymentMethod.BANK.value, PaymentMethod.CHEQUE.value, PaymentMethod.ONLINE.value]
            for i in range(pay_n):
                method = methods[i % len(methods)]
                currency = currencies[i % len(currencies)]
                target_type = i % 6
                kwargs = {}
                if target_type == 0 and created_sales:
                    sale = created_sales[i % len(created_sales)]
                    kwargs = {"entity_type": PaymentEntityType.SALE.value, "sale_id": sale.id}
                elif target_type == 1 and created_services:
                    srv = created_services[i % len(created_services)]
                    kwargs = {"entity_type": PaymentEntityType.SERVICE.value, "service_id": srv.id}
                elif target_type == 2 and created_invoices:
                    inv = created_invoices[i % len(created_invoices)]
                    kwargs = {"entity_type": PaymentEntityType.INVOICE.value, "invoice_id": inv.id}
                elif target_type == 3 and created_preorders:
                    pre = created_preorders[i % len(created_preorders)]
                    kwargs = {"entity_type": PaymentEntityType.PREORDER.value, "preorder_id": pre.id}
                elif target_type == 4 and created_shipments:
                    shp = created_shipments[i % len(created_shipments)]
                    kwargs = {"entity_type": PaymentEntityType.SHIPMENT.value, "shipment_id": shp.id}
                elif target_type == 5 and created_suppliers:
                    sup = created_suppliers[i % len(created_suppliers)]
                    kwargs = {"entity_type": PaymentEntityType.SUPPLIER.value, "supplier_id": sup.id}
                if not kwargs and created_customers:
                    cust = created_customers[i % len(created_customers)]
                    kwargs = {"entity_type": PaymentEntityType.CUSTOMER.value, "customer_id": cust.id}
                direction = PaymentDirection.OUT.value if kwargs.get("entity_type") in (PaymentEntityType.SHIPMENT.value, PaymentEntityType.SUPPLIER.value) else PaymentDirection.IN.value
                pay = Payment(
                    payment_date=now - timedelta(days=i),
                    total_amount=_Q2(50 + i * 5),
                    currency=currency,
                    method=method,
                    status=PaymentStatus.COMPLETED.value,
                    direction=direction,
                    reference=_ref("PAY", i),
                    notes=f"Seed payment {seed_tag}",
                    receiver_name=str(user.username or "user"),
                    deliverer_name=f"deliverer {seed_tag}",
                    created_by=user.id,
                    **kwargs,
                )
                if method == PaymentMethod.CHEQUE.value:
                    pay.check_number = _ref("CHK", i)
                    pay.check_bank = "Seed Bank"
                    pay.check_due_date = now + timedelta(days=10)
                if method == PaymentMethod.CARD.value:
                    pay.card_holder = "Seed Card"
                    pay.card_last4 = f"{(1000 + i) % 9999:04d}"
                    pay.card_expiry = "12/30"
                if method == PaymentMethod.BANK.value:
                    pay.bank_transfer_ref = _ref("BNK", i)
                if kwargs.get("entity_type") == PaymentEntityType.SHIPMENT.value:
                    pay.direction = PaymentDirection.OUT.value
                    pay.reference = pay.reference or "Seed shipment payment"
                db.session.add(pay)
            _flush("payments")
            ...
            for i in range(chk_n):
                ...
                chk = Check(
                    amount=_Q2(80 + i * 4),
                    currency=currency,
                    check_number=_ref("C", i),
                    check_bank="Seed Bank",
                    check_date=now - timedelta(days=i),
                    check_due_date=now + timedelta(days=7 + i),
                    status=CheckStatus.PENDING.value,
                    direction=PaymentDirection.IN.value if i % 2 == 0 else PaymentDirection.OUT.value,
                    customer_id=created_customers[i % len(created_customers)].id if created_customers else None,
                    supplier_id=created_suppliers[i % len(created_suppliers)].id if created_suppliers else None,
                    partner_id=created_partners[i % len(created_partners)].id if created_partners else None,
                    reference_number=f"Seed check {seed_tag}",
                    notes=f"Seed check {seed_tag}",
                    created_by_id=user.id,
                )
                db.session.add(chk)
            _flush("checks")
        ...
        for s in created_suppliers:
            try:
                update_supplier_balance_components(s.id)
            except Exception:
                pass
        for p in created_partners:
            try:
                update_partner_balance_components(p.id)
            except Exception:
                pass
        db.session.commit()
        ...
        click.echo(json.dumps({
            "seed_tag": seed_tag,
            "customers": [c.id for c in created_customers],
            "suppliers": [s.id for s in created_suppliers],
            "partners": [p.id for p in created_partners],
            "products": [p.id for p in created_products],
            "sales": [s.id for s in created_sales],
            "services": [s.id for s in created_services],
            "shipments": [s.id for s in created_shipments],
            "expenses": [e.id for e in created_expenses],
            "invoices": [i.id for i in created_invoices],
            "preorders": [p.id for p in created_preorders],
        }, ensure_ascii=False))
    except SQLAlchemyError as e:
        db.session.rollback()
        raise click.ClickException(f"Commit failed: {e}") from e
...
def register_cli(app) -> None:
    commands=[
        ...
        seed_employees, seed_salaries, seed_expenses_demo, seed_customer_statement_demo, seed_deep_demo, seed_branches,
