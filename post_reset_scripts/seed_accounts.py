...
from models import GL_ACCOUNTS, ExpenseType, EquipmentType, SystemSettings, func

def seed_accounts(app=None):
    """
    Seed chart of accounts and basic expense types.
    ...
    with app.app_context():
        ...
        expense_types_data = [
            ...
            ("RENT", "إيجار", "إيجار المحل"),
            ("UTILITIES", "مرافق", "كهرباء، مياه، إنترنت"),
            ("MAINTENANCE", "صيانة", "صيانة المحل والمعدات"),
            ("FUEL", "وقود", "وقود السيارات"),
            ("OFFICE", "مكتب", "قرطاسية ومستلزمات مكتبية"),
            ("GOV_FEES", "رسوم حكومية", "تراخيص وضرائب ورسوم"),
            ("MARKETING", "تسويق", "إعلانات ودعاية"),
            ("HOSPITALITY", "ضيافة", "ضيافة للعملاء والموظفين"),
            ("OTHER", "أخرى", "مصاريف متنوعة"),
        ]
        
        added_types = 0
        for code, name, desc in expense_types_data:
            ...
            if not existing:
                ...
                new_type = ExpenseType(
                    ...
                    description=desc,
                    is_active=True,
                    # default_account_code is not available in ExpenseType model currently. 
                    # Skipped mapping to avoid error. Logic for default account should be handled in Expense creation logic or updated model.
                )
                db.session.add(new_type)
                added_types += 1
        
        db.session.commit()
        print(f"✅ Seeded {added_types} expense types.")

        # --- SEED EQUIPMENT TYPES (Vehicles) ---
        print("🌱 Seeding Equipment Types...")
        EQUIPMENT_TYPES_DEFAULT = [
            "سيارة ركاب (Sedan)", "دفع رباعي (SUV)", "شاحنة خفيفة (Pickup)", 
            "شاحنة ثقيلة (Truck)", "حافلة (Bus)", "معدات ثقيلة", "أخرى"
        ]
        
        added_eq = 0
        for name in EQUIPMENT_TYPES_DEFAULT:
            existing = EquipmentType.query.filter(func.lower(EquipmentType.name) == name.lower()).first()
            if not existing:
                eq = EquipmentType(name=name, category="VEHICLE")
                db.session.add(eq)
                added_eq += 1
        
        db.session.commit()
        print(f"✅ Seeded {added_eq} equipment types.")
        ...
        settings_defaults = {
            ...
            'theme_font_family': "'Cairo', 'Tajawal', sans-serif",
            'system_default_language': 'ar',
        ...
        db.session.commit()
        print(f"✅ Seeded {added_settings} system settings.")
