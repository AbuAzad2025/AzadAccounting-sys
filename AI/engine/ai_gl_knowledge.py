...
GL_SYSTEM_KNOWLEDGE = {
    ...
    "auto_gl_creation": {
        ...
        "modules": [
            ...
            {"module": "Sale", "entries": "ذمم العملاء ↔ إيرادات/ضريبة/تكلفة حسب الإعدادات"},
...
GL_BUSINESS_RULES = {
    ...
    "sale_accounting": {"rule": "المبيعات تسجل عند تحقق شروط الاعتماد/التأكيد في النظام.", "basic": "ذمم العملاء مدين، والإيرادات/الضريبة دائن حسب الإعدادات."},
...
def explain_any_number(number_value: float, context: str) -> str:
    ...
    if context == "customer_balance":
        ...
        return f"""📊 **رصيد العميل: {val:,.2f} ₪**

هذا الرقم يمثل رصيد العميل حسب سياسة النظام الحالية.
اتجاه الرصيد: {direction}.

مصادره المحتملة:
- مبيعات أو فواتير على العميل
- دفعات واردة أو مردودات
- رصيد افتتاحي
- قيود GL مرتبطة بالعميل

لا أفترض هنا أن الموجب يعني عليه أو له دائمًا؛ يجب اعتماد تفسير النظام المعروض في صفحة العميل أو سياسة الرصيد المحاسبية."""
...
def trace_transaction_flow(transaction_type: str, transaction_id: int) -> Dict[str, Any]:
    flows = {
        "sale": {
            "step_1": {"title": "تسجيل البيع", "table": "Sale/SaleLine", "data": ["العميل", "المنتجات", "الكميات", "الأسعار", "الإجمالي"]},
            "step_2": {"title": "تحديث المخزون", "table": "StockLevel", "formula": "الكمية الجديدة = الكمية السابقة - الكمية المباعة"},
            "step_3": {"title": "إنشاء القيد المحاسبي", "table": "GLBatch/GLEntry", "entries": ["ذمم العملاء مدين", "الإيراد دائن", "الضريبة دائن إذا كانت مفعلة", "التكلفة/المخزون حسب إعدادات النظام"]},
            "step_4": {"title": "تحديث رصيد العميل", "calculation": "حسب سياسة الرصيد المعتمدة في النظام"},
