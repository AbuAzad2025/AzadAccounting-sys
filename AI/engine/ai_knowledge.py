...
class SystemKnowledgeBase:
    ...
    def index_all_files(self, force_reindex: bool = False):
        ...
        # Rebuild index sections while preserving counters and old quality until recalculated.
    ...
    def calculate_learning_quality(self):
        """Calculate a conservative quality index from available real data only."""
        try:
            ...
            learning_quality = sum(components) / len(components) if components else 0
            ...
            self.knowledge["learning_quality"] = {
                ...
                "data_density": round(data_density_score, 2) if data_density_score is not None else None,
                "system_health": round(system_health_score, 2),
                "tables_with_data": tables_with_data,
                "total_critical_tables": total_critical_tables,
                "note": "No fabricated confidence defaults are used.",
            }
        except Exception:
    ...
    def extract_currency_rules(self):
        self.knowledge["business_rules"].extend(
            [
                ...
                {"rule": "سعر الصرف يجب أن يُقرأ من آخر عملية/إعداد فعلي وليس من قيمة ثابتة", "source": "exchange_transactions/settings", "impact": "high"},
    ...
    def index_models(self):
        ...
        try:
            ...
            for match in re.finditer(class_pattern, content, re.MULTILINE):
                ...
                is_enum = "Enum" in inheritance or "enum" in inheritance
            ...
            for folder in ("routes", "services", "AI/engine"):
                ...
                if directory.exists():
                    for py_file in directory.rglob("*.py"):
                        ...
                        self.knowledge["functions"][str(py_file)] = {"functions": funcs[:120], "classes": classes[:50]}
            for main_file in ["app.py", "utils.py", "validators.py", "acl.py", "config.py"]:
                path = self.base_path / main_file
                if path.exists():
                    content = path.read_text(encoding="utf-8")
                    self.knowledge["functions"][main_file] = {"functions": re.findall(r"^def\s+(\w+)\s*\(", content, re.MULTILINE)[:120]}
        except Exception:
            pass
    ...
    def index_templates(self):
        ...
        try:
            for template_file in templates_dir.rglob("*.html"):
                module_name = str(template_file.parent.relative_to(templates_dir)) if template_file.parent != templates_dir else "root"
                self.knowledge["templates"].setdefault(module_name, []).append(template_file.name)
        except Exception:
            pass
    ...
    def index_javascript(self):
        ...
        try:
            for js_file in js_dir.rglob("*.js"):
                ...
                names = [a or b for a, b in functions if a or b]
                events = re.findall(r"addEventListener\([\"'](\w+)[\"']", content)
                self.knowledge["javascript"][str(js_file.relative_to(self.base_path))] = {"functions": names[:80], "events": sorted(set(events))[:30]}
        except Exception:
            pass
    ...
    def index_css(self):
        ...
        try:
            for css_file in css_dir.rglob("*.css"):
                ...
                self.knowledge["css"][str(css_file.relative_to(self.base_path))] = {"classes": sorted(set(css_classes))[:150], "ids": sorted(set(css_ids))[:80]}
        except Exception:
            pass
    ...
    def analyze_relationships(self):
        ...
        for model_name, model_data in self.knowledge.get("models", {}).items():
    ...
    def get_accounting_knowledge(self):
        return {
            "principles": {
                "double_entry": "كل قيد محاسبي يجب أن يكون متوازنًا: مجموع المدين = مجموع الدائن",
                "balance_sheet": "الأصول = الخصوم + حقوق الملكية",
                "income_statement": "صافي الربح = الإيرادات - المصروفات",
            },
            "notes": ["هذه معرفة عامة؛ الأرقام والحسابات النهائية يجب أن تُقرأ من قاعدة النظام الفعلية."],
        }
...
class ErrorAnalyzer:
    def analyze_traceback(self, traceback_text: str) -> Dict[str, Any]:
        ...
        patterns = [
            ("ImportError", "مشكلة في الاستيراد أو مكتبة غير مثبتة", "تحقق من requirements.txt والبيئة الافتراضية"),
            ("ModuleNotFoundError", "مكتبة أو ملف غير موجود", "ثبت المكتبة أو صحح مسار الاستيراد"),
            ("AttributeError", "خاصية أو دالة غير موجودة على الكائن", "تحقق من نوع الكائن واسم الخاصية"),
            ("KeyError", "مفتاح غير موجود في dict", "استخدم get() أو تحقق من وجود المفتاح"),
            ("TypeError", "نوع بيانات غير مناسب للعملية", "تحقق من التحويلات وأنواع المتغيرات"),
            ("ValueError", "قيمة غير صالحة", "أضف validation أو try/except"),
            ("IntegrityError", "مخالفة قيد في قاعدة البيانات", "تحقق من التكرار والمفاتيح الأجنبية والحقول المطلوبة"),
            ("OperationalError", "مشكلة اتصال أو تشغيل قاعدة البيانات", "تحقق من DATABASE_URL وحالة قاعدة البيانات"),
        ]
    ...
    def format_error_response(analysis: Dict[str, Any]) -> str:
        ...
        return f"""
{severity_emoji.get(analysis.get('severity'), '⚠️')} **خطأ: {analysis.get('error_type', 'Unknown')}**
...
def get_local_faq_responses():
    return {
        "من أنت": "أنا المساعد الذكي داخل نظام أزاد. أساعدك في التنقل، فهم البيانات، المحاسبة، التقارير، والأخطاء البرمجية داخل النظام.",
        "ما قدراتك": "أستطيع قراءة خريطة النظام، فهم الجداول والنماذج، تحليل الأخطاء، شرح القيود المحاسبية، وتوجيهك للصفحات المناسبة حسب الصلاحيات.",
        "كيف أضيف عميل": "افتح صفحة العملاء ثم إضافة عميل جديد. أدخل الاسم والهاتف والبيانات المطلوبة، ثم احفظ. تحقق من الرصيد الافتتاحي قبل الحفظ.",
        "أين النفقات": "صفحة النفقات تكون عادة عبر /expenses، ويمكنك من هناك إضافة أو مراجعة المصروفات حسب صلاحيتك.",
    }


def get_local_quick_rules():
    return {
        "count_customers": {"patterns": ["كم عدد العملاء", "عدد الزبائن", "how many customers"], "model": "Customer", "response_template": "✅ عدد العملاء: {count} عميل"},
        "count_services": {"patterns": ["كم صيانة", "عدد الصيانات", "طلبات الصيانة"], "model": "ServiceRequest", "response_template": "🔧 عدد طلبات الصيانة: {count} طلب"},
        "count_expenses": {"patterns": ["كم نفقة", "عدد النفقات", "المصاريف"], "model": "Expense", "response_template": "💸 عدد النفقات: {count} نفقة"},
        "count_products": {"patterns": ["كم منتج", "عدد القطع", "المنتجات"], "model": "Product", "response_template": "📦 عدد المنتجات: {count} منتج"},
        "count_suppliers": {"patterns": ["كم مورد", "عدد الموردين"], "model": "Supplier", "response_template": "🏭 عدد الموردين: {count} مورد"},
    }
