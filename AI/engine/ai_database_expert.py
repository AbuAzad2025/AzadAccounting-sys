...
BUSINESS_DOMAINS = {
    "customers": {"keywords": ["عميل", "عملاء", "زبون", "customer", "client", "رصيد عميل", "كشف عميل"], "models": ["Customer", "Invoice", "Sale", "Payment", "ServiceRequest"], "expert_focus": ["الاسم والهاتف لتجنب التكرار", "الرصيد لا يفسر دون سياسة النظام", "كشف العميل يجمع الفواتير والدفعات والصيانة"]},
    "suppliers": {"keywords": ["مورد", "موردين", "supplier", "vendor", "كشف مورد"], "models": ["Supplier", "Payment", "SupplierSettlement", "Product"], "expert_focus": ["مراجعة المستحقات والتسويات", "تمييز المورد عن العميل", "ربط المورد بالمشتريات أو المنتجات عند توفر العلاقة"]},
    "sales": {"keywords": ["بيع", "مبيعات", "sale", "sales", "فاتورة", "invoice"], "models": ["Sale", "SaleLine", "Invoice", "Payment", "Customer", "StockLevel"], "expert_focus": ["العميل والمستودع والكمية قبل الحفظ", "VAT من الإعدادات فقط", "راجع أثر البيع على المخزون والذمة"]},
...
class DatabaseExpert:
    ...
    def _workflow_for_domain(self, domain: str) -> List[str]:
        mapping = {
            "customers": ["ابحث عن العميل", "راجع كشفه", "راجع آخر فاتورة/دفعة", "ثم أجب أو وجّه المستخدم"],
            "suppliers": ["ابحث عن المورد", "راجع التسويات/الدفعات", "راجع العملة والرصيد", "ثم أجب"],
            "sales": ["حدد الفترة والعميل", "اجمع الفواتير/المبيعات", "راجع الحالة والضريبة", "اربط الدفعات إن لزم"],
            ...
            "services": ["حدد العميل/المركبة", "راجع حالة الطلب", "راجع القطع والتكلفة", "راجع الدفعات قبل الإغلاق"],
    ...
    def _mistakes_for_domain(self, domain: str) -> List[str]:
        mapping = {
            "customers": ["تكرار العميل", "تفسير الرصيد عكس السياسة", "إهمال الهاتف في البحث"],
            "suppliers": ["خلط المورد مع العميل", "تجاهل التسوية", "عكس الرصيد"],
            ...
            "services": ["إغلاق الطلب قبل التحصيل", "عدم خصم القطع", "خلط التشخيص بوصف العميل"],
